...
def set_df():
    ...
    for tab_name in tabs_list:
        ...
        if df is None:
            ...
            df_temp = pd.read_excel('load.xlsx', skiprows=4, engine='openpyxl')
...
def create_new_excel(name, inserted_data):
    global number_label
    global table
    global df
    wb = Workbook()
...
def add_row(parent_func, cemetery, inserted_data, name):
    ...
    if not os.path.exists(PATH + '/' + name):
        create_new_excel(name, inserted_data)
        return 0
        ...
        for i in range(2, 9):
            if str(datetime.now().year) + '_' + str(cemetery) + '_' + str(i) + '.xlsx' in tabs_list:
                ...
                if ws['A' + str(ws.max_row)].value + 1 > 5000:
                    ...
                    add_row(cemetery, inserted_data, str(datetime.now().year) + '_' + str(cemetery) + '_' + str(i) + '.xlsx')
                    break
            elif str(datetime.now().year) + '_' + str(cemetery) + '_' + str(i) + '.xlsx' not in tabs_list:
                create_new_excel(str(datetime.now().year) + '_' + str(cemetery) + '_' + str(i) + '.xlsx', inserted_data)
                break                
        return 0
    ws.append(inserted_data)
...
def add_fio_old():
    ...
    if inserted_data[2] != '' and inserted_data[3] != '' and age > -1 and cemetery_is_set:
        try:
            ...
            create_new_excel('2011' + '_' + str(cemetery) + '.xlsx', inserted_data)  
...
def add_fio():
    ...
    if inserted_data[2] != '' and inserted_data[3] != '' and age > -1 and cemetery_is_set:
        try:
            ...
            create_new_excel(str(datetime.now().year) + '_' + str(cemetery) + '.xlsx', inserted_data)       
...
def open_main_window():
    ...
    notebook.add(frame_add, text="Добавление новых лет")
    ...
    last_name_entry_add_old = tk.Entry(frame_add_old)
    ...
    name_entry_add_old = tk.Entry(frame_add_old)
    ...
    surname_entry_add_old = tk.Entry(frame_add_old)
    ...
    age_entry_old = tk.Entry(frame_add_old)
    ...
    death_date_entry_old = tk.Entry(frame_add_old)
    ...
    certificate_number_entry_old = tk.Entry(frame_add_old)
    ...
    given_entry_old = tk.Entry(frame_add_old)
    ...
    burial_entry_old = tk.Entry(frame_add_old)
    ...
    cemetery_entry_old = tk.Entry(frame_add_old)
    ...
    sector_number_entry_old = tk.Entry(frame_add_old)
    ...
    row_entry_old = tk.Entry(frame_add_old)
    ...
    place_number_entry_old = tk.Entry(frame_add_old)
    ...
    size_entry_old = tk.Entry(frame_add_old)
    ...
    digger_entry_old = tk.Entry(frame_add_old)
    ...
    responsibility_entry_old = tk.Entry(frame_add_old)
