...
def add_row(parent_func, cemetery, inserted_data, name):
    ...
    if inserted_data[0] > 5000:
        wb.close()
        for i in range(2, 9):
            if year + '_' + str(cemetery) + '_' + str(i) + '.xlsx' in tabs_list:
                wb = openpyxl.load_workbook(year + '_' + str(cemetery) + '_' + str(i) + '.xlsx')
                ws = wb.active
                if ws['A' + str(ws.max_row)].value + 1 > 5000:
                    continue
                else:
                    add_row(parent_func, cemetery, inserted_data, year + '_' + str(cemetery) + '_' + str(i) + '.xlsx')
                    break
            elif year + '_' + str(cemetery) + '_' + str(i) + '.xlsx' not in tabs_list:
                create_new_excel(parent_func, year + '_' + str(cemetery) + '_' + str(i) + '.xlsx', inserted_data)
                break                
        return 0
    ws.append(inserted_data)
